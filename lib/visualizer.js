...
module.exports = function printResult(db, last_update, clioptions) {
  var invoices = db('invoices');
  var payments = db('payments');

  var revenue = getRevenue(invoices, clioptions.year);
  var revenueByMonth = getRevenueByFormat(invoices, clioptions.year, revenue, 'MM', 12);
  var revenueByQuarter = getRevenueByFormat(invoices, clioptions.year, revenue, 'Q', 4);

  var incomeByMonth = getIncomeByFormat(payments, clioptions.year, 'MM', 12);
  var incomeByQuarter = getIncomeByFormat(payments, clioptions.year, 'Q', 4);

  var taxesByQuarter = getTaxesByFormat(invoices, clioptions.year, revenue, 'Q', 4);
  ...
  var revenueByClient = getRevenueByClient(invoices, clioptions.year, revenue);
  ...
  incomeTaxesTable.setData({
    headers: ['Q', '€'],
    data: _.map(incomeByQuarter, (el)=> [el.name, formatNumber(el.amount*vat/100)])
  });
  ...
  clientTable.setData({
    headers: ['Client', ' %', '€'],
    data: _.map(revenueByClient, (el)=> [el.name, el.percentage, formatNumber(el.amount)])
  });
